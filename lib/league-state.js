...
async function leagueState(leagueId) {
  ...
  const remaining = [];
  // Who has already played whom. Strength of schedule needs the fixtures
  // behind a team as well as the ones in front of it, and the played half is
  // the half that explains the record it already has.
  const playedPairs = [];
  let weeksPlayed = 0;
  ...
  allWeeks.forEach((rows, i) => {
    ...
    playedPairs.push({ week: i + 1, pairs });
    weeksPlayed++;
  ...
  return {
    ...
    teams, remaining, playedPairs, playoffTeams, weeksPlayed, lastRegular,
    leagueMean, meanFrom,
  };
}
