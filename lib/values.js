...
/** The most a player who is NOT ranked could be worth, per position. */
function unrankedCeilingFrom(priced) {
  const out = {};
  for (const row of priced) {
    const cur = out[row.pos];
    if (cur === undefined || row.vorp < cur) out[row.pos] = row.vorp;
  }
  for (const pos of Object.keys(out)) out[pos] = Math.max(0, out[pos]);
  return out;
}
...
async function valueTable() {
  ...
  // WHAT AN UNRANKED PLAYER CAN BE WORTH, AT MOST.
  //
  // "No published value" is true and, on its own, useless: measured against a
  // real 10-team keeper league, 83 of 189 rostered players are unranked, so a
  // calculator that refuses to judge any trade containing one refuses to judge
  // most trades. But unranked is not unknown — the ranking is a complete top-N
  // at each position, so a player left off it is, by the analyst's own account,
  // below the last man on it. That is a CEILING, and it is a tight one: the
  // last ranked receiver is worth 4 points above replacement and the last
  // ranked quarterback is below replacement entirely.
  //
  // Floored at zero because a below-replacement player costs nothing to hold:
  // he is droppable, so nobody trades him at a negative price. That keeps the
  // bound conservative in the direction that matters — it can only ever
  // OVERSTATE what the unknown side is worth, never understate it.
  const unrankedCeiling = unrankedCeilingFrom(priced);

  const data = {
    generated: new Date().toISOString(),
    // Per position: the most a player who is NOT ranked could be worth, taken
    // from the last man who is. Used to decide whether a trade's outcome is
    // determined despite the players it could not price.
    unrankedCeiling,
    format: FORMAT,
...
module.exports = { valueTable, FORMAT, slotValues, replacementMedians, verifyAgainstOverall, unrankedCeilingFrom, REPLACEMENT_RANK };
